...
export interface DitherEffectProps {
  waveSpeed?: number;
  waveFrequency?: number;
  waveAmplitude?: number;
  waveColor?: [number, number, number];
...
export default function ClientOnlyDither({
  waveSpeed = 0.05,
  waveFrequency = 3,
  waveAmplitude = 0.3,
  ...
  if (!mounted) {
    return <div className="w-full h-full bg-red-500" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
      Loading Dither...
    </div>;
  }

  if (error) {
    return <div className="w-full h-full bg-yellow-500" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'black' }}>
      Error: {error}
    </div>;
  }
  ...
  try {
    ...
    console.error('Dither Canvas Error:', err);
    return <div className="w-full h-full bg-orange-500" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white' }}>
      Canvas Error: {err instanceof Error ? err.message : 'Unknown error'}
    </div>;
  }
}